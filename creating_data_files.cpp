...
vector<PrimaryIndex> buildPrimaryIndexLength(const string &dataFile,
    ...
    while (getline(in, line)) {
        size_t lineLenWithNewline = line.length() + (size_t)newlineBytes();

        if (!line.empty()) {
            bool isDeleted = (line[0] == '*');
            auto fields = split(line);
            if (!isDeleted && fields.size() > idFieldIndex) {
                PrimaryIndex entry;
                strcpy(entry.recordID, fields[idFieldIndex].c_str());
                entry.offset = (int)currentOffset;
                entry.recordLength = (int)lineLenWithNewline;
                primaryIndex.push_back(entry);
            }
        }

        currentOffset += (long long)lineLenWithNewline;
